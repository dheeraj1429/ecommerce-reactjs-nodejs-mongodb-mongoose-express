...
function DashboardProductsComponent() {
  ...
  if (selector !== null && selector.success === true) {
    filterData = selector.AllProducts.filter((el) => el.name.toLowerCase().includes(SeachData.toLowerCase()));
  }
                ...
                <div className="py-3">
                  <div className="row">
                    {selector !== null && selector.success === true && filterData
                      ? filterData.map(({ _id, ...otherProps }) => <ProductCardComponent id={_id} {...otherProps} key={_id} />)
                      : null}
                  </div>
                </div>
              </div>
